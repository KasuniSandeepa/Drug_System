...
<head>
    ...
    <title>IMPASL Drug | ISD Approve</title>
...
<div class="app-admin-wrap layout-sidebar-large">
 ...
 -->
  <div>
	...
	<li class="nav-item">
		<a class="nav-link   active {{request () -> is('/ISDForm2.jsp') ? 'active' : null }}" href="ISDRecords.jsp" role="tab">ISD Data Entry</a>
	</li>
	<li class="nav-item">
                ...
                <div class="col-md-12 mb-4">
                    <div class="card text-left">
                        <div class="card-body">
                            ...
                            <span class=" mb-3">TOTAL NO OF RECORDS RETRIEVED : <span id="count"><%=drugList.size()%></span></span>
<a href="ISDFormEntry.jsp"><input id="g" type="button" class="btn btn-primary"
										name="" value="Add a new ISD Record" style="float: right;"/></a>	
                            <div class="table-responsive">
                                <table class="display table table-striped table-bordered"  id="deafult_ordering_table_aa"
                                  ...
                                  <% for(ISD drug : drugList){%>
                                        ...
                                        <td><%=drug.getGENERIC()%></td>
										<td><%=drug.getBRAND()%></td>
										<td><%=drug.getFORMULATION()%></td>
										<td><%=drug.getSTRENGTH()%></td>
										<td><%=drug.getIMPORTER()%></td>
										<td><%=drug.getMANUFACTURER()%></td>
			                            
			                            <%if(drug.getSTATUS()==null || drug.getSTATUS().equals("0")) {%>
			                            
			                          <%--  <form action="controller/DrugContrller.jsp">
			                         
			                            <input name="action" value="approved" hidden>
			                               <input name="drugId" value='<%=drug.getID() %>' hidden>
			                                  <td><!-- <button type="submit" style="color:red" onclick="approve()">Approve</button> -->
			                         ...
			                         <td> <button type="button" class="btn btn-labeled btn-danger">
                							Pending</button></td>
			                            <%}else if( drug.getSTATUS().equals("1")){ %>
			                            <%--   <form action="controller/DrugContrller.jsp">
          								...
          								Approved</button></td>
			                               ...
			                               <%}else if( drug.getSTATUS().equals("2")){ %>
			                                <td> <button type="button" class="btn btn-labeled btn-warning">
          								Rejected</button></td>
          								<%} %>
			                                  <td>
			                                  
				                            <c:url value="EditISDFormEntry.jsp" var="updatedDrug">
											<c:param name="id" value='<%=drug.getID()%>'/>
											<c:param name="generic" value='<%=drug.getGENERIC()%>' />
											<c:param name="generic2" value='<%=drug.getGENERIC2()%>' />
											<c:param name="generic3" value='<%=drug.getGENERIC3()%>' />
											<c:param name="brand" value='<%=drug.getBRAND()%>' />
											<c:param name="dosage" value='<%=drug.getFORMULATION()%>' />
											<c:param name="strength" value='<%=drug.getSTRENGTH()%>' />
											<c:param name="imc_code" value='<%=drug.getIMC_CODE()%>' />
											<c:param name="importer" value='<%=drug.getIMPORTER()%>' />
											<c:param name="manufacturer" value='<%=drug.getMANUFACTURER()%>' />
											<c:param name="expireDate" value='<%=drug.getEXPIRE_DATE()%>' />
											<c:param name="shedule" value='<%=drug.getSHEDULE()%>' />
											<c:param name="country" value='<%=drug.getCOUNTRY()%>' />
											<c:param name="notes" value='<%=drug.getNOTES()%>' />
											<c:param name="status" value='<%=drug.getSTATUS()%>' />
											<%-- <c:param name="name" value="${name}" />
											<c:param name="mobileNumber" value="${mobileNumber}" />
											<c:param name="operator" value="${operator}" />
											<c:param name="age" value="${age}" />
											<c:param name="gender" value="${gender}" />
											<c:param name="dob" value="${dob}" />
											<c:param name="createdDate" value="${createdDate}" /> --%>
										</c:url>
			                                  	...
			                                  	<a href="${updatedDrug}"><input type="button" class="btn btn-primary"
